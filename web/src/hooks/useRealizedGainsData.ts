...
export function useGainsTableData({ realizedSales }: UseGainsTableDataArgs) {
  ...
  const availableYears = useMemo(() => {
    const years = new Set(realizedSales.map((s) => s.date.slice(0, 4)));
  ...
  const totalPages = Math.max(1, Math.ceil(filteredGains.length / PAGE_SIZE));
  const safePage = Math.min(page, totalPages);
  const pagedGains = filteredGains.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE);
...
export function useDividendsTableData({ dividendsReceived }: UseDividendsTableDataArgs) {
  ...
  const availableYears = useMemo(() => {
    const years = new Set(dividendsReceived.map((d) => d.date.slice(0, 4)));
    return Array.from(years).sort((a, b) => b.localeCompare(a));
  }, [dividendsReceived]);
  ...
  const totalPages = Math.max(1, Math.ceil(filteredDividends.length / PAGE_SIZE));
  const safePage = Math.min(page, totalPages);
  const pagedDividends = filteredDividends.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE);
