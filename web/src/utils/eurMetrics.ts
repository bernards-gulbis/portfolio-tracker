import type { Holding, PortfolioStatus } from '../api';

export interface EurMetrics {
  available: boolean;
  rate: number;
  currentValueEur: number;
  unrealizedGainsEur: number;
...
/**
 * Compute all EUR-denominated portfolio metrics from a PortfolioStatus + live rate.
 ...
 */
export const computeEurMetrics = (status: PortfolioStatus): EurMetrics | null => {
  const rate = status.usd_to_eur_rate;
  if (rate === null || rate === undefined) return null;

  const currentValueEur = status.current_value * rate;
  ...
  const skipTax = status.dividends > 0 && status.dividends_eur === null;
 ...
 */
export const applyRateToHolding = (holding: Holding, rate: number): HoldingEurValues => ({
