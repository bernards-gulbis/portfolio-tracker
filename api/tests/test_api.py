...
def test_create_portfolio(client: TestClient):
    ...
    response = client.post("/portfolios/", json={"name": "My Investment Portfolio"})
...
def test_get_portfolio(client: TestClient):
    ...
    # Create portfolio
    create_response = client.post("/portfolios/", json={"name": "Test Portfolio"})
...
def test_update_portfolio(client: TestClient):
    ...
    # Create portfolio
    create_response = client.post("/portfolios/", json={"name": "Original Name"})
    portfolio_id = create_response.json()["id"]

    # Update portfolio
    response = client.put(f"/portfolios/{portfolio_id}", json={"name": "Updated Name"})
    assert response.status_code == 200
    data = response.json()
...
def test_delete_portfolio(client: TestClient):
    ...
    # Create portfolio
    create_response = client.post("/portfolios/", json={"name": "To Delete"})
    portfolio_id = create_response.json()["id"]
...
def test_create_transaction(client: TestClient):
    ...
    # Create portfolio
    portfolio_response = client.post("/portfolios/", json={"name": "Test Portfolio"})
...
def test_list_transactions_filter_by_ticker(client: TestClient):
    ...
    # Exact ticker match
    response = client.get(
        f"/portfolios/{portfolio_id}/transactions", params={"ticker": "MSFT"}
    )
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 1
    ...
    response = client.get(
        f"/portfolios/{portfolio_id}/transactions", params={"ticker": "ms"}
    )
    data = response.json()
    assert data["total"] == 1
    assert data["transactions"][0]["ticker"] == "MSFT"
    ...
    # No match
    response = client.get(
        f"/portfolios/{portfolio_id}/transactions", params={"ticker": "GOOG"}
    )
    data = response.json()
    assert data["total"] == 0
...
def test_list_transactions_filter_by_type(client: TestClient):
    ...
    # Filter by Buy
    response = client.get(
        f"/portfolios/{portfolio_id}/transactions", params={"type": "Buy"}
    )
    data = response.json()
    assert data["total"] == 1
    assert data["transactions"][0]["type"] == "Buy"
    ...
    # Filter by Deposit
    response = client.get(
        f"/portfolios/{portfolio_id}/transactions", params={"type": "Deposit"}
    )
    data = response.json()
    assert data["total"] == 2
...
def test_list_transactions_filter_combined(client: TestClient):
    ...
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Combined Filter Test"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
def test_update_transaction(client: TestClient):
    ...
    # Update transaction
    response = client.put(
        f"/transactions/{transaction_id}", json={"total_amount": 3500.00, "fee": 10.0}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_export_transactions_csv(client: TestClient):
    ...
    # Create portfolio
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Export Test Portfolio"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
def test_export_and_reimport_csv(client: TestClient):
    ...
    # Create portfolio
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Export-Import Test"}
    )
    portfolio_id = portfolio_response.json()["id"]
    ...
    portfolio2_response = client.post(
        "/portfolios/", json={"name": "Imported Portfolio"}
    )
    ...
    import_response = client.post(
        f"/portfolios/{portfolio2_id}/transactions/import", files=files
    )
    ...
    transactions_response = client.get(
        f"/portfolios/{portfolio2_id}/transactions"
    ).json()
...
def test_export_transactions_csv_empty(client: TestClient):
    ...
    portfolio_response = client.post("/portfolios/", json={"name": "Empty Portfolio"})
    portfolio_id = portfolio_response.json()["id"]
...
def test_csv_upload(client: TestClient):
    ...
    # Create portfolio
    portfolio_response = client.post(
        "/portfolios/", json={"name": "CSV Test Portfolio"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
"""
    ...
    files = {"file": ("transactions.csv", BytesIO(csv_content.encode()), "text/csv")}
    response = client.post("/portfolios/999/transactions/import", files=files)
...
def test_csv_with_complex_transactions(client: TestClient):
    ...
    # Create portfolio
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Complex CSV Portfolio"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
def test_csv_upload_invalid_fx_rate(client: TestClient):
    ...
    # Create portfolio
    portfolio_response = client.post(
        "/portfolios/", json={"name": "FX Rate Validation Test"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
"""
    files = {
        "file": ("transactions.csv", BytesIO(csv_content_zero.encode()), "text/csv")
    }
    response = client.post(
        f"/portfolios/{portfolio_id}/transactions/import", files=files
    )
...
"""
    files = {
        "file": ("transactions.csv", BytesIO(csv_content_negative.encode()), "text/csv")
    }
    response = client.post(
        f"/portfolios/{portfolio_id}/transactions/import", files=files
    )
...
def test_copy_portfolio_with_transactions(client: TestClient):
    ...
    # Create portfolio
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Original Portfolio"}
    )
    portfolio_id = portfolio_response.json()["id"]
    ...
    copy_response = client.post(
        f"/portfolios/{portfolio_id}/copy", json={"new_name": "Copied Portfolio"}
    )
    ...
    original_tx_resp = client.get(
        f"/portfolios/{portfolio_id}/transactions", params={"sort_order": "asc"}
    )
    ...
    copied_tx_resp = client.get(
        f"/portfolios/{copied_portfolio_id}/transactions", params={"sort_order": "asc"}
    )
    ...
    assert (
        original_transactions[0]["total_amount"]
        == copied_transactions[0]["total_amount"]
    )
...
def test_copy_portfolio_not_found(client: TestClient):
    ...
    response = client.post("/portfolios/999/copy", json={"new_name": "Copy of Missing"})
    assert response.status_code == 404
...
def test_copy_portfolio_invalid_name(client: TestClient):
    ...
    # Create portfolio
    portfolio_response = client.post("/portfolios/", json={"name": "Original"})
    portfolio_id = portfolio_response.json()["id"]

    # Try to copy with empty name
    response = client.post(f"/portfolios/{portfolio_id}/copy", json={"new_name": "   "})
    assert response.status_code == 400
...
def test_portfolio_status_with_buy_transactions(client: TestClient):
    ...
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Investment Portfolio"}
    )
    portfolio_id = portfolio_response.json()["id"]
    ...
    assert data["cash"] == pytest.approx(
        2000.0
    )  # 5000 - 1500 - 1500 (fees included in values)
...
def test_portfolio_status_with_sell_transactions(client: TestClient):
    ...
    portfolio_response = client.post("/portfolios/", json={"name": "Trading Portfolio"})
    portfolio_id = portfolio_response.json()["id"]
    ...
    assert data["cash"] == pytest.approx(
        9500.0
    )  # 10000 - 2000 + 1500 (fees included in values)
...
def test_portfolio_status_with_dividends(client: TestClient):
    ...
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Dividend Portfolio"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
def test_portfolio_status_with_stock_split(client: TestClient):
    ...
    portfolio_response = client.post("/portfolios/", json={"name": "Split Portfolio"})
    portfolio_id = portfolio_response.json()["id"]
...
def test_portfolio_status_with_withdrawal(client: TestClient):
    ...
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Withdrawal Portfolio"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
def test_portfolio_status_invalid_split_ratio(client: TestClient):
    ...
    assert (
        split_response.status_code == 422
...
def test_portfolio_status_with_eur_conversion(client: TestClient):
    ...
    portfolio_response = client.post(
        "/portfolios/", json={"name": "EUR Test Portfolio"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
def test_portfolio_status_with_positive_capital_gains_tax(client: TestClient):
    ...
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Tax Test Portfolio"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
def test_portfolio_status_tax_excludes_dividends(client: TestClient):
    ...
    portfolio_response = client.post("/portfolios/", json={"name": "Dividend Tax Test"})
    portfolio_id = portfolio_response.json()["id"]
...
def test_portfolio_status_tax_none_when_dividend_eur_unavailable(client: TestClient):
    ...
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Missing Dividend EUR Test"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
def test_portfolio_status_eur_none_when_exchange_rate_unavailable(client: TestClient):
    ...
    portfolio_response = client.post(
        "/portfolios/", json={"name": "No Exchange Rate Test"}
    )
    portfolio_id = portfolio_response.json()["id"]
    ...
    assert data["principal_eur"] == pytest.approx(
        10000.0
...
def test_portfolio_status_eur_conversion_with_different_rates(client: TestClient):
    ...
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Rate Difference Test"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
def test_cross_user_portfolio_write_returns_404(session: Session):
    ...
    try:
        ...
        assert (
            client.put(
                f"/portfolios/{portfolio.id}", json={"name": "Renamed"}
            ).status_code
            == 404
        )
...
def test_register_duplicate_email_returns_400(session: Session):
    ...
    try:
        ...
        response = client.post(
            "/auth/register",
            json={"email": "dup@example.com", "password": "different123"},
        )
        assert response.status_code == 400
    finally:
        app.dependency_overrides.clear()
...
def test_performance_chart_split_adjusted_prices(client: TestClient):
    ...
    # Create portfolio
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Split Performance Test"}
    )
    portfolio_id = portfolio_response.json()["id"]
    ...
    def mock_historical_prices(
        tickers, start_date, end_date, max_workers=5, per_ticker_start=None
    ):
        result = {}
...
def test_performance_chart_multiple_splits(client: TestClient):
    ...
    portfolio_response = client.post("/portfolios/", json={"name": "Multi-Split Test"})
    portfolio_id = portfolio_response.json()["id"]
...
def test_performance_chart_no_splits(client: TestClient):
    ...
    portfolio_response = client.post("/portfolios/", json={"name": "No Split Test"})
    portfolio_id = portfolio_response.json()["id"]
...
def test_performance_chart_missing_price_fallback(client: TestClient):
    ...
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Missing Price Test"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
def test_performance_last_known_price_gap(client: TestClient):
    ...
    portfolio_response = client.post("/portfolios/", json={"name": "Price Gap Test"})
    portfolio_id = portfolio_response.json()["id"]
...
def test_performance_delisted_db_fallback(client: TestClient):
    ...
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Delisted DB Fallback Test"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
def test_performance_delisted_no_cache(client: TestClient):
    ...
    portfolio_response = client.post(
        "/portfolios/", json={"name": "No Cache Fallback Test"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
def test_performance_twr_mid_period_deposit(client: TestClient):
    ...
    """
    portfolio_response = client.post(
        "/portfolios/", json={"name": "TWR Mid-Deposit Test"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
def test_performance_twr_price_gain(client: TestClient):
    ...
    """
    portfolio_response = client.post("/portfolios/", json={"name": "TWR Gain Test"})
    portfolio_id = portfolio_response.json()["id"]
...
def test_portfolio_status_uses_custom_tax_rate(
    ...
    portfolio_response = client.post(
        "/portfolios/", json={"name": "Custom Tax Portfolio"}
    )
    portfolio_id = portfolio_response.json()["id"]
...
class TestTransactionRouterExtraPaths:
    ...
    def test_import_unicode_decode_error_returns_400(self, client):
        """Uploading a non-UTF-8 file triggers UnicodeDecodeError and returns 400."""
        c, _session, _user = client
        pid = self._create_portfolio(c)
    ...
    @pytest.mark.anyio
    async def test_import_generic_read_error_raises_file_upload_exception(self, client):
    ...
    def test_update_transaction_type_field_rename(self, client):
